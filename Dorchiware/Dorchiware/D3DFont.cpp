...
HRESULT CD3DFont::InitializeDeviceObjects( LPDIRECT3DDEVICE9 pd3dDevice )
{
	...
	hr = m_pD3DDevice->CreateTexture(m_dwTexWidth, m_dwTexHeight, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_pTexture, NULL);
	...
	for (y = 0; y < m_dwTexHeight; y++) {
		...
		for (x = 0; x < m_dwTexWidth; x++) {
			BYTE bAlpha = (BYTE)(pBitmapBits[m_dwTexWidth * y + x] >> 8) & 0xFF;
			*pDst32 = 0xFFFFFFFF;
			((BYTE*)pDst32)[3] = bAlpha;
